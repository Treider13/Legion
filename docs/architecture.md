...
│  cmd (UART/WS/BLE, cmd-мьютекс) → synth (единый мьютекс ADF4351) │
│  sweepTask (FreeRTOS-тики: SWEEP/HOP/CHIRP/GLIDE/FM) · telemTask │
│  NVS: автономный рестарт · SELFTEST · web_server (LittleFS)      │
└───────────────┬──────────────────────────────────────────────────┘
                │ SPI bit-bang (SFR GPIO), запись строго R5→R0
┌───────────────▼──────────────────────────────────────────────────┐
│  ADF4351 (TCXO 25 МГц) → RF out 34.375–4400 МГц, −4…+5 дБм       │
└──────────────────────────────────────────────────────────────────┘
```
...
5. **Единый мьютекс `synth`** — нет гонок ручной режим ↔ коридор (от первичного плана «очередь + synthTask» отказались сознательно: мьютекс даёт ту же эксклюзивность с меньшим джиттером перестройки).
