...
- **ТИП СИГНАЛА** (режим SDR) — 31 baseband-волна: синус, тон Fj + случ. фаза,
  ...
  «ЗАШИТЬ НА SDR» — TX-петля воркера (fs 2 МГц, гетеродин +fs/8), только в
  нагрузку 50 Ом и полосу allowlist. Зашитая волна становится TX-контентом
  всех режимов: АВТО/ПРИОРИТЕТ (сканер → цель) и open-loop (качание/сплошная/
  случайная) идут с ней, пока оператор не сбросит на CW
