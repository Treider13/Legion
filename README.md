...
| FPGA-ревизия legion | `fpga/` | ревизия `legion` для bladeRF 2.0 micro xA4/xA9 и bladeRF 1 x40: автономный тракт в FPGA (плеер RAM / NCO / loopback по детектору), watchdog, агент шлюза, NIOS-подъём эфира на micro (AIR-регистры); онбордовый перехват — плата смотрит эфир и шагает LO сама, USB не в круге «увидел → усилитель»; вендоренное дерево Nuand в `fpga/vendor/` — сборка из репозитория; сборка и запись образа из приложения — вкладка **КАСТОМ FPGA** (СОБРАТЬ → ПРОШИТЬ); см. `fpga/README.md` |
...
- **Два режима (не смешивать):**
  1. **SDR (Ethernet)** — каталог bladeRF (x40, micro xA4/xA9) / HackRF /
     ...
     **«Автоматический перехват»**): после Старта хозяин — SDR.
     ...
     Гейт в текущем взгляде — микросекунды; обзор коридора — шаги LO (десятки МГц
     за взгляд), миллисекунды. USB не в круге «увидел → усилитель» (см. `fpga/README.md`).
