...
частоты **Analog Devices ADF4351** (35 МГц – 4.4 ГГц) через микроконтроллер
...
- **MANUAL** — точная частота 35–4400 МГц, подтверждение захвата ФАПЧ (LOCK),
...
- **ТИП СИГНАЛА** (режим SDR) — 16 baseband-волн: синус, тон Fj + случ. фаза,
  меандр, пила, треугольник, чирп, AWGN, BPSK/QPSK/16-QAM + RRC, 2-FSK (CPFSK),
  DSSS (m-seq Gp=31), CSS (LoRa-подобный), OFDM (802.11a-подобный), AM, FM.
  Предпросмотр спектра/созвездия в UI; «ЗАШИТЬ НА SDR» — TX-петля воркера
  (fs 2 МГц, гетеродин +fs/8), только в нагрузку 50 Ом и полосу allowlist
...
|---|---|---|
| Математика регистров | `cd firmware && pio test -e native` | 23 юнит-теста против эталонов даташита/pyadf435x (сетка 35–4400 МГц, «проклятая» 2175 МГц) |
