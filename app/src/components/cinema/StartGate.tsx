...
import { type CinemaMode, runSimpleStart, runSmartStart } from "./run";
...
export function StartGate({ mode, onClose }: Props) {
  ...
  const continueRun = async () => {
    ...
    try {
      if (mode === "sdr") {
        await runSmartStart({ f1, f2, wave, loadOk: ohm });
      } else {
    ...
    <div className="cinema-gate" role="presentation" onClick={(e) => e.target === e.currentTarget && !busy && onClose()}>
      <div className="cinema-gate-card" role="dialog" aria-modal="true" aria-labelledby={titleId}>
        ...
        <p className="cinema-gate-lead">
          {mode === "sdr"
            ? "Скан идёт по F1–F2. Засечка уходит на усилитель выбранной волной. Задержка — µs хоста, не FPGA."
        ...
        <div className="cinema-gate-actions">
          ...
          <button type="button" className="cinema-btn solid" onClick={() => void continueRun()} disabled={busy}>
            {busy ? "…" : "Продолжить"}
          </button>
        </div>
      </div>
