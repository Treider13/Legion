...
import { computePosition, fresnelRadiusM, gasDbPerKm, knifeEdgeDb } from "../src/sense/position/compute";
...
test("45 км, 2,4 ГГц, гребень 220 м: вся трасса, не только середина", () => {
  ...
  assert.equal(r.rx1, "Засечёте на RX1.");
  assert.ok((r.marginDb ?? -999) > 0);
});
...
test("22 ГГц и клетка 30 м: мало данных, RX1 эту частоту не принимает, газ есть", () => {
  ...
  assert.match(r.action, /Клетка карты/);
  assert.match(r.rx1 ?? "", /не засечёте/);
